...
proc beginFlow(s: Start): Middle {.transition.} =
  Middle(SimpleFlow())

proc finishFlow(m: Middle): End {.transition.} =
  End(SimpleFlow())
...
proc succeed(b: Begin): Success {.transition.} =
  Success(MultiTerm())

proc fail(b: Begin): Failure {.transition.} =
  Failure(MultiTerm())
...
proc pause(a: Active): Paused {.transition.} =
  Paused(Resettable())

proc resume(p: Paused): Active {.transition.} =
  Active(Resettable())

proc stopFromActive(a: Active): Stopped {.transition.} =
  Stopped(Resettable())

proc stopFromPaused(p: Paused): Stopped {.transition.} =
  Stopped(Resettable())
