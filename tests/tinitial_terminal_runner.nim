...
proc testTransitionToInitial(): bool =
  ...
  let (output, exitCode) = execCmdEx("nim c --hints:off tests/should_fail/states/transition_to_initial.nim")

  if exitCode == 0:
    echo "  FAIL: Compilation should have failed but succeeded"
    return false

  if "initial state" in output.toLowerAscii or "cannot transition to" in output.toLowerAscii or "cannot declare transition to" in output.toLowerAscii:
...
proc testTransitionFromTerminal(): bool =
  ...
  let (output, exitCode) = execCmdEx("nim c --hints:off tests/should_fail/states/transition_from_terminal.nim")

  if exitCode == 0:
    echo "  FAIL: Compilation should have failed but succeeded"
    return false

  if "terminal state" in output.toLowerAscii or "cannot transition from" in output.toLowerAscii or "cannot declare transition from" in output.toLowerAscii:
