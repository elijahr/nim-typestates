...
block syntaxErrorTest:
  let (output, exitCode) = execCmdEx("nim c -r --hints:off --path:src src/nim_typestates_bin.nim verify tests/fixtures/syntax_error.nim 2>&1")
...
block multiFileTest:
  let (output, exitCode) = execCmdEx("nim c -r --hints:off --path:src src/nim_typestates_bin.nim dot tests/fixtures/basic_typestate.nim tests/fixtures/with_flags.nim 2>&1")
...
block dotOutputTest:
  let (output, exitCode) = execCmdEx("nim c -r --hints:off --path:src src/nim_typestates_bin.nim dot tests/fixtures/branching_transitions.nim 2>&1")
