...
block explicitRaisesTest:
  let (output, exitCode) = execCmdEx("nim c tests/traises_enforcement/explicit_raises.nim")
  ...
  if "non-empty raises" in lowerOutput or "raises: []" in lowerOutput or "error state" in lowerOutput:
...
block implicitRaisesTest:
  let (output, exitCode) = execCmdEx("nim c tests/traises_enforcement/implicit_raises.nim")
