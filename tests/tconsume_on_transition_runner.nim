...
let (output, exitCode) =
  execCmdEx("nim c --hints:off tests/should_fail/consume/consume_on_transition.nim")

if exitCode == 0:
  echo "FAIL: Compilation should have failed but succeeded"
  quit(1)

if "cannot copy" in output.toLowerAscii or "=copy" in output.toLowerAscii or
    "is not available" in output.toLowerAscii:
